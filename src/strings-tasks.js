...
function sumOfCodes(str) {
  return !str
    ? 0
    : str.split('').reduce((acc, curr) => acc + curr.charCodeAt(), 0);
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((acc, curr) => (vowels.includes(curr) ? acc + 1 : acc + 0), 0);
}
...
function isPalindrome(str) {
  const str2 = str
    ...
    .reverse()
    .join('');
  const str1 = str.toLowerCase().replaceAll(/[^a-z]/g, '');
...
function findLongestWord(sentence) {
  const arr = [];
  sentence.split(' ').map((item) => arr.push(item.length));
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const index = (alphabet.indexOf(str[i].toLowerCase()) + 13) % 26;
